...
function isSimpleDOMProps(
  props: SimpleDOMPropsIF | ReactNode
): props is SimpleDOMPropsIF {
  return !!(props && (props as object).hasOwnProperty("types"));
}

function transformCode(code: (SimpleDOMPropsIF | ReactNode)[]) {
  return function Tag(
    Fn: React.FunctionComponent<any> | React.ComponentClass<any>,
    Props = {}
  ): SimpleDOMPropsIF | ReactNode[] {
    return code.map((item, index) => {
      if (isSimpleDOMProps(item)) {
        return (
          <Fn key={index} name={item.types} {...item.props} {...Props}>
            {transformCode(item.children)(Fn, Props)}
          </Fn>
        );
      }
      return item;
...
export default function text2RE(
  Fn: React.FunctionComponent<any> | React.ComponentClass<any>
) {
  return function <P>(props: Readonly<{ children?: ReactNode } & P>) {
    ...
    return <Fragment>{transformCode(code)(Fn, others)}</Fragment>;
  };
}
